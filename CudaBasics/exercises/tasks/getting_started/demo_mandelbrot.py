...
parser.add_option("-b", "--benchmark", dest = "isBenchmark", help = "run program without graphical"
...
def calc_fractal_opencl(q, maxiter,ctx):
...
def calc_fractal_serial(q, maxiter,ctx=None):
...
def calc_fractal_numpy(q, maxiter,ctx=None):
...
if len(args) > 0:
    #if args[0] not in ['gpu', 'gpuarray', 'numpy', 'python']:
    if args[0] not in ['gpu', 'cpu', 'numpy', 'python']:
        show_instructions = True
...
if show_instructions:
    print "Usage: python mandelbrot.py [gpu|cpu|numpy|python]"
    print "Where:"
    print " gpu is a pure OpenCL solution on the GPU"
    print " cpu is a pure OpenCL solution on the CPU"
...
routine = {'cpu':calc_fractal_opencl,
            'gpu':calc_fractal_opencl,
            'numpy':calc_fractal_numpy,
            'python':calc_fractal_serial}

calc_fractal = routine[args[0]]
...
if __name__ == '__main__':
    ...
    class Mandelbrot(object):
        def __init__(self):
            if not options.isBenchmark:
                ...
                for p in cl.get_platforms():
                    if args[0] == 'cpu':
                        try:
                            device = p.get_devices(cl.device_type.CPU)
                            break
                        except cl.RuntimeError, e:
                            pass
                    elif args[0] == 'gpu':
                        try:
                            device = p.get_devices(cl.device_type.GPU)
                            break
                        except cl.RuntimeError, e:
                            pass
                if device:
                    self.ctx = cl.Context(device)
                elif args[0] in ('gpu', 'cpu'):
                    print "Couldn't create OpenCL context for %s.", args[0]
                    sys.exit(1)
        ...
        def draw(self, x1, x2, y1, y2, maxiter=300):
            ...
            if calc_fractal == calc_fractal_opencl:
                if not options.isBenchmark:
                    print "Doing a warm up."
                output = calc_fractal(q, maxiter, self.ctx)
