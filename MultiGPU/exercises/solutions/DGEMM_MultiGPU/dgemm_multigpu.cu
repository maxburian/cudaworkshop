...
void matrixMulOnDevice(const double *a, const double* b, double *c, int n) { 
  ...
  dim3 gridDim( (n + BLOCK_SIZE - 1) / blockDim.x, (ns + BLOCK_SIZE - 1) / blockDim.y); 
  
  size = n * n * sizeof (double);
  strip_size = ns * n * sizeof (double);

  // Allocate memory for d_a0, d_b0 and d_c0 on device
  cudaSetDevice(0);
  cudaMalloc((void**)&d_a0, strip_size);
  checkError("allocating device memory for A");
  cudaMalloc((void**)&d_b0, size);
  checkError("allocating device memory for B");
  cudaMalloc((void**)&d_c0, strip_size);
  checkError("allocating device memory for C");

  // Allocate memory for d_a0, d_b0 and d_c0 on device 
  cudaSetDevice(1);
  cudaMalloc((void**)&d_a1, strip_size);
  checkError("allocating device memory for A");
  cudaMalloc((void**)&d_b1, size);
  checkError("allocating device memory for B");
  cudaMalloc((void**)&d_c1, strip_size);
  checkError("allocating device memory for C");

  // Copy data for a and b from host to device and start measuring
  gettimeofday(&start, NULL);
  cudaSetDevice(0);
  cudaMemcpy(d_a0, a, strip_size, cudaMemcpyHostToDevice); 
  ...
  cudaSetDevice(1);
  cudaMemcpy(d_a1, &a[ns*n], strip_size, cudaMemcpyHostToDevice);
  checkError("copying data of A from host to device");
  cudaMemcpy(d_b1, b, size, cudaMemcpyHostToDevice);
  checkError("copying data of B from host to device");
  ...
  cudaSetDevice(0);
  cudaMemcpy(c, d_c0, strip_size, cudaMemcpyDeviceToHost); 
  checkError("copying results from device to host");
  
  cudaSetDevice(1);
  cudaMemcpy(&c[ns*n], d_c1, strip_size, cudaMemcpyDeviceToHost);
