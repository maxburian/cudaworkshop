...
#ifdef OVERLAPP_TRANSFERS
cudaStream_t copy_streams[MAX_GPUS];
#endif
...
void jacobi()
{
    while (residue > tol && iter < iter_max)
    {
        ...
        if ( size == 2 )
        {
            ...
            CUDA_CALL( cudaMemcpyAsync ( u_d[1]+0*m+1, unew_d[0]+(n-2)*m+1, (m-2)*sizeof(float), cudaMemcpyDeviceToDevice, copy_streams[1] ) );
            //2. Copy first modified row of GPU 1 to lower boundary of GPU 0:
            CUDA_CALL( cudaMemcpyAsync ( u_d[0]+(n-1)*m+1, unew_d[1] + 1*m+1, (m-2)*sizeof(float), cudaMemcpyDeviceToDevice, copy_streams[0] ) );
#else
            //1. Copy last modified row of GPU 0 to upper boundary of GPU 1:
            CUDA_CALL( cudaMemcpy ( u_d[1]+0*m+1, unew_d[0]+(n-2)*m+1, (m-2)*sizeof(float), cudaMemcpyDeviceToDevice ) );
            //2. Copy first modified row of GPU 1 to lower boundary of GPU 0:
            CUDA_CALL( cudaMemcpy ( u_d[0]+(n-1)*m+1, unew_d[1] + 1*m+1, (m-2)*sizeof(float), cudaMemcpyDeviceToDevice ) );
#endif
        }
...
#pragma omp parallel for
...
#ifdef OVERLAPP_TRANSFERS
        CUDA_CALL( cudaStreamCreate ( &(copy_streams[rank]) ) );
#endif
    }
    ...
    {
        int canAccessPeer = 0;
        CUDA_CALL( cudaDeviceCanAccessPeer ( &canAccessPeer, 0, 1 ) );
        if ( canAccessPeer )
        {
...
#ifdef OVERLAPP_TRANSFERS
        CUDA_CALL( cudaStreamDestroy( copy_streams[rank] ) );
#endif
    }
